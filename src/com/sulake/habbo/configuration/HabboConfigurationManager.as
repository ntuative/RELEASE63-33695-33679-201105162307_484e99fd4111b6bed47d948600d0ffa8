package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1718);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "HabboConfigurationCom_default_configuration")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "HabboConfigurationCom_default_configuration")
         {
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         ...
         if(this.var_2050.indexOf(param1) < 0 || param3)
         {
            this.var_1065[param1] = param2;
         }
         if(param3)
         {
            this.var_2050.push(param1);
         }
      }
      ...
      public function getBoolean(param1:String, param2:Boolean) : Boolean
      {
         ...
         return _loc3_ == "1" || _loc3_.toLowerCase() == "HabboConfigurationCom_default_configuration";
      }
   }
}
