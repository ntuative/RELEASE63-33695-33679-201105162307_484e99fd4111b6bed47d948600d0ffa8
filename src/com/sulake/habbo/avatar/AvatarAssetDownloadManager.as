package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_969 = new Timer(this.const_2223,1);
         this.var_969.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_445)
         {
            ...
            this.var_445.addEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_445)
         {
            ...
            this.var_445.removeEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_968);
            if(this.var_1465.indexOf("?") > 0)
            {
               _loc2_ = this.var_1465 + "&retry=" + this.var_968;
            }
            else
            {
               _loc2_ = this.var_1465 + "?retry=" + this.var_968;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2537,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1187[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1187[_loc5_] = _loc6_;
            }
         }
      }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_970[_loc3_];
         }
         while(_loc6_ < this.var_774.length)
         {
            _loc5_ = this.var_774[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_774.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_969.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_775.length > 0 && this.var_774.length < this.const_1686)
         {
            _loc1_ = this.var_775[0];
            _loc1_.startDownloading();
            this.var_774.push(this.var_775.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_775.indexOf(param1) == -1 && this.var_774.indexOf(param1) == -1)
         {
