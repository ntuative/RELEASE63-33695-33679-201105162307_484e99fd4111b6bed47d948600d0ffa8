package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_814:IFigurePartSet;
      ...
      private var var_660:Rectangle;
      
      private var var_1939:int = 0;
      
      private var var_455:IAvatarRenderManager;
      
      private var var_1090:Boolean;
      
      private var var_1938:BitmapData;
      
      private const const_2230:int = 13421772;
      
      private const const_2229:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1939 = Math.max(this.var_1939,_loc6_.colorLayerIndex);
            }
         }
         this.var_455 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_705 != null && !this.var_1937)
            {
               _loc3_ = this.var_705;
            }
            else
            ...
            if(this.var_814)
            {
               switch(this.var_814.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_926;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_455.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_455.isFigureReady(_loc1_))
         {
            this.var_455.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_364[_loc6_] + "_" + FigureData.const_149;
               _loc4_ = this.var_455.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_660 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1938)
               {
                  ...
                  this.var_1938 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_660.width,this.var_660.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_660.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_660.y;
               if(this.var_1937 && _loc2_.colorLayerIndex > 0)
               {
